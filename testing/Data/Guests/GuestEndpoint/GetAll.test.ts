import { assert } from "chai";
import axios, { AxiosResponse } from "axios";

const API_URL = "http://localhost:3000/api/";

describe("Testing Guests '/getAll/' endpoint", () => {
    let data: AxiosResponse<any, any>;

    beforeEach(async() => {
        data = await axios.get( API_URL+"guests/getAll" );
    });

    it("data shouldn't be of type 'undefined'", donne => {
        assert.notEqual( data, undefined );
        donne();
    });

    it("should return a 200 code", donne => {
        assert.equal( data.status, 200 );
        donne();
    });


    it("should return an array of type Guest", donne => {
        assert.typeOf( data.data, 'array' );
        donne();
    });

    it("should have same size as described in /count/ endpoint", async() => {
        return await axios.get( API_URL+"guests/getCount" ).then((res) => {
            assert.equal( data.data.length, res.data );
        });
    });
});
