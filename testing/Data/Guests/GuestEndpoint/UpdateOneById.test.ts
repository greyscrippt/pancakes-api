...
describe("Testing '/updateOneById' endpoint", () => {
    ...
    beforeEach(async() => {
        const sample = await axios.get( API_URL+"guests/getAll" );
        ...
        data_to_be_updated = await axios.get( API_URL+"guests/getOneById/"+sample.data[0]._id );

        let updated_data_array = {"name": "Whatever New Name"};

        updated_data_response = await axios.patch(
            API_URL+"guests/updateOneById/"+data_to_be_updated.data._id,
            updated_data_array,
        );
    });
