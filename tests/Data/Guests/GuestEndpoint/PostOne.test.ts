...
describe("GuestController.postOne()", async() => {
    let data: AxiosResponse<any, any>;

    beforeEach(async() => {
        data = await axios.post( API_URL+"guests/postOne", { name: "My Very Own Test" } );
    });
    ...
    it("the result data should be gettable from /getOne/ endpoint", async() => {
        const data = axios.post( API_URL+"guests/postOne", { name: "My Very Own Test" } );
